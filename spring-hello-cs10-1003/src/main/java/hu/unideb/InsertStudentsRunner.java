...
public class InsertStudentsRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
         System.out.println("justlikemefrfr");
         final var faker = new Faker();

         final var programs = Arrays.stream(Student.Program.values()).collect(Collectors.toList());
         ...
         for(int i = 0; i < 100; i++){
             ...
             final var student = new Student(
                     IntStream.range(0, 6)
                             .mapToObj(pos -> pos == 0?LETTERS.charAt(RANDOM.nextInt(26))
                             : (LETTERS + DIGITS).charAt(RANDOM.nextInt(36))).map(String::valueOf)
